...
cards.forEach(card => {
  const a = document.createElement("a");
  a.className = "card " + (card.type === "big" ? "big" : "");
  a.href = card.url || "#";
  if (card.newTab) { a.target = "_blank"; a.rel = "noopener"; }
  ...
  if (card.action === "copy") {
    a.addEventListener("click", e => {
      e.preventDefault();
      navigator.clipboard.writeText(card.copyText).then(() => showToast("Copied: " + card.copyText));
    });
  }
...
document.getElementById("shareBtn").addEventListener("click", () => {
  if (navigator.share) {
    navigator.share({ title: "Nicole Gan", text: "INTCO Medical - Account Manager", url: location.href });
  } else {
